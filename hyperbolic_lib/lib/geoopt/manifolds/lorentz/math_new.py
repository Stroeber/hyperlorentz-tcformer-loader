...
def _parallel_transport0(y, v, k, dim: int = -1):
    ...
    denom_b = _norm(nomin, keepdim=True)
    lmap_b =  dist_ * nomin / denom_b

    p = v - nom / denom * (lmap + lmap_b)
    return p
...
def _parallel_transport0back(x, v, k, dim: int = -1):
    ...
    denom = _norm(nomin, keepdim=True)
    lmap_b = dist_ * nomin / denom

    # lmap = _logmap0back(x, k=k, dim=dim)
    nom = _inner(lmap_b, v, keepdim=True)
    denom = dist_** 2

    nomin_ = inner_x * torch.sqrt(k)
    dn = x.size(dim) - 1
    nomin = torch.cat((nomin_ + x.narrow(dim, 0, 1), x.narrow(dim, 1, dn)), dim)
    denom_2 = _norm(nomin, keepdim=True)
    lmap = dist_ * nomin / denom_2

    p = v - nom / denom * (lmap_b + lmap)
    return p
